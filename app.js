...
// -------------------------------------------------------------
// NEW: BLAZING FAST CONCURRENT DATA LOADING
// -------------------------------------------------------------
async function loadAllData() {
    ...
    try {
        // Step 1: Fire all 4 Google Sheets requests at the exact same time
        ...
        // Step 2: Parse all the responses into JSON at the exact same time
        const [opData, docData, volData, trainData] = await Promise.all([
            (opRes && opRes.ok) ? opRes.json() : Promise.resolve([]),
            (docRes && docRes.ok) ? docRes.json() : Promise.resolve([]),
            (volRes && volRes.ok) ? volRes.json() : Promise.resolve([]),
            (trainRes && trainRes.ok) ? trainRes.json() : Promise.resolve([])
        ]);

        // Step 3: Assign the data to our global vaults securely
        if (opData && !opData.error) rawOperationsData = opData;
        if (docData && !docData.error) rawDocumentsData = docData;
        if (volData && !volData.error) rawVolunteersData = volData;
        if (trainData && !trainData.error) rawTrainingsData = trainData;

        // Step 4: Process everything for the dashboard
        ...
        // Step 5: Smoothly hide the loader and reveal the dashboard!
        hideLoader();
