...
// ==========================================
// FLAWLESS GLOBAL CALENDAR TOOLTIP
// ==========================================
document.addEventListener("DOMContentLoaded", function() {
    // 1. Create the global tooltip container and attach it directly to the body
    const globalTooltip = document.createElement('div');
    globalTooltip.id = 'global-cal-tooltip';
    document.body.appendChild(globalTooltip);

    // 2. Listen for mouse hovering over calendar events
    document.addEventListener('mouseover', function(e) {
        const dayEl = e.target.closest('.cal-day.has-event');
        if (dayEl) {
            // Find the hidden tooltip data generated by your calendar loop
            const hiddenTooltip = dayEl.querySelector('.cal-tooltip');
            if (hiddenTooltip) {
                // Copy the content into our global floating box
                globalTooltip.innerHTML = hiddenTooltip.innerHTML;
    ...
    document.addEventListener('mousemove', function(e) {
        if (globalTooltip.classList.contains('active')) {
            // Position the tooltip 15px below and to the right of the mouse cursor
            let x = e.pageX + 15;
            let y = e.pageY + 15;
            
            // Smart Boundary Detection: Prevents tooltip from flying off the right/bottom edges of the screen
            const rect = globalTooltip.getBoundingClientRect();
            if (x + rect.width > window.innerWidth + window.scrollX) {
                x = e.pageX - rect.width - 15; // Flip to the left side of the cursor
            }
            if (y + rect.height > window.innerHeight + window.scrollY) {
                y = e.pageY - rect.height - 15; // Flip above the cursor
            }
    ...
    // 4. Hide the tooltip when the mouse leaves the calendar day
