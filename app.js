...
let currentPieState = { level: 1, filterKey: 'all', level1Target: null, level2Target: null };
...
const sharedTooltipConfig = {
    backgroundColor: 'rgba(30, 41, 59, 0.95)', titleColor: '#ffffff', bodyColor: '#ffffff',
    titleFont: { family: 'Inter', size: 11, weight: '800' }, bodyFont: { family: 'Inter', size: 11, weight: '600' },
    padding: 10, cornerRadius: 6, displayColors: false, borderColor: 'rgba(255, 255, 255, 0.4)', borderWidth: 1, caretSize: 6, caretPadding: 6
};
...
function extractYear(row, type) {
    if (type === 'doc') {
        ...
        if (dStr) { let d = parseCustomDate(dStr); if (d) return d.getFullYear().toString(); }
    } else if (type === 'op') {
...
function hideLoader() {
    ...
    if (loader) {
        ...
        setTimeout(() => { loader.style.visibility = 'hidden'; loader.style.display = 'none'; }, 800); 
    }
}
...
async function fetchOpenWeather(cityQuery) {
    ...
    try {
        ...
        if (currentRes.ok && forecastRes.ok) {
            ...
            const rainLabels = []; const rainDataPoints = [];
            for(let i = 0; i < 5; i++) {
                const item = forecastData.list[i];
                const date = new Date(item.dt * 1000);
                let hour = date.getHours(); let ampm = hour >= 12 ? 'PM' : 'AM'; hour = hour % 12 || 12; 
...
function updateRainChart(labels, dataPoints) {
    ...
    rainChartInstance = new Chart(ctx, {
        type: 'bar',
        data: { labels: labels, datasets: [{ data: dataPoints, backgroundColor: 'rgba(14, 165, 233, 0.7)', borderWidth: 0, borderRadius: 4 }] },
        options: {
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { display: false }, tooltip: { enabled: false }, datalabels: { display: true, color: '#ffffff', anchor: 'end', align: 'top', font: { size: 10, weight: 'bold', family: 'Inter' }, formatter: (value) => value + '%' } },
            scales: { y: { display: false, min: 0, max: 100 }, x: { ticks: { color: '#94a3b8', font: { size: 9, weight: 'bold', family: 'Inter' } }, grid: { display: false }, border: { display: false } } },
...
// ==========================================
// CORE DATA FETCHING & FILTERING
// ==========================================
async function loadAllData() {
    if (!webAppUrl || webAppUrl === "YOUR_NEW_WEB_APP_URL_HERE") {
        console.error("Please add your Web App URL to app.js"); hideLoader(); return;
    }
    try {
        const [opRes, docRes, volRes, trainRes] = await Promise.all([
            fetch(`${webAppUrl}?type=operations`).catch(() => null),
            fetch(`${webAppUrl}?type=documents`).catch(() => null),
            fetch(`${webAppUrl}?type=volunteers`).catch(() => null),
            fetch(`${webAppUrl}?type=trainings`).catch(() => null)
        ]);

        if (opRes && opRes.ok) { const data = await opRes.json(); if (!data.error) rawOperationsData = data; }
        if (docRes && docRes.ok) { const data = await docRes.json(); if (!data.error) rawDocumentsData = data; }
        if (volRes && volRes.ok) { const data = await volRes.json(); if (!data.error) rawVolunteersData = data; }
        if (trainRes && trainRes.ok) { const data = await trainRes.json(); if (!data.error) rawTrainingsData = data; }

        let yearsSet = new Set();
        rawOperationsData.forEach(r => { let y = extractYear(r, 'op'); if (y && !isNaN(y)) yearsSet.add(y); });
        rawDocumentsData.forEach(r => { let y = extractYear(r, 'doc'); if (y && !isNaN(y)) yearsSet.add(y); });
        ...
        if (yearSelect) {
            ...
            if (yearsSet.has(currentYear)) yearSelect.value = currentYear;
        }
    ...
    } catch (error) { console.error("Error fetching secure data:", error); hideLoader(); }
}

function applyGlobalYearFilter(targetYear) {
    let filteredOps = rawOperationsData; let filteredDocs = rawDocumentsData;
    if (targetYear !== 'all') {
    ...
    operationsMonthlyCache = {}; globalLineData = []; globalDocRecords = []; originalKPITotals = {};
...
// ==========================================
// MISSING CHART HELPER FUNCTIONS RESTORED
// ==========================================
function renderTrendFooter(elementId, dataArray, labelsArray, inverseColors = false) {
    ...
    let currentLabel = 'Current Month'; let prevLabel = 'Previous Month';

    if (dataArray.length >= 2 && labelsArray.length >= 2) {
        current = dataArray[dataArray.length - 1]; previous = dataArray[dataArray.length - 2];
        currentLabel = labelsArray[labelsArray.length - 1]; prevLabel = labelsArray[labelsArray.length - 2];
    } else if (dataArray.length === 1) {
        current = dataArray[0]; currentLabel = labelsArray[0];
    }

    const diff = current - previous;
    let trendHtml = ''; let bgColor = '#64748b'; 

    if (dataArray.length < 2) {
        el.style.backgroundColor = bgColor; el.style.padding = '10px 16px'; 
        el.innerHTML = `<div style="font-weight:600; font-size:0.75rem; color:#fff;"><span>No prior data</span></div>`;
        return;
    }

    let symbol = '—'; let sign = diff > 0 ? '+' : '';
    ...
    if (diff > 0) { symbol = arrowUp; bgColor = inverseColors ? '#ef4444' : '#10b981'; } 
    else if (diff < 0) { symbol = arrowDown; bgColor = inverseColors ? '#10b981' : '#ef4444'; sign = '-'; }
    ...
    el.style.backgroundColor = bgColor; el.style.padding = '10px 16px'; el.style.color = '#ffffff';
    el.innerHTML = `
...
function renderLineChartByTimeframe(timeframe) {
    ...
    sortedData.forEach(item => {
        let key = "";
        if (timeframe === 'monthly') { key = item.dateObj.toLocaleString('en-US', { month: 'short', year: 'numeric' }); } 
        else if (timeframe === 'yearly') { key = item.dateObj.getFullYear().toString(); } 
        else { key = item.dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }); }
        groupedObj[key] = (groupedObj[key] || 0) + item.count;
    });

    const labels = Object.keys(groupedObj); const dataValues = Object.values(groupedObj);
    if(labels.length === 0) { drawLineChart('docDateLineChart', ['No Date Data Found'], [0]); } 
    else { drawLineChart('docDateLineChart', labels, dataValues); }
}
...
function drawLineChart(canvasId, labels, dataArr) {
    ...
    docLineChartInstance = new Chart(ctx, {
        type: 'line',
        data: { labels: labels, datasets: [{ label: 'Requests Received', data: dataArr, borderColor: '#2563eb', backgroundColor: gradient, borderWidth: 2, pointRadius: 0, pointHoverRadius: 5, pointBackgroundColor: '#ffffff', pointBorderColor: '#2563eb', pointBorderWidth: 2, tension: 0.4, fill: true }] },
        options: { 
            ...
            scales: { x: { grid: { display: false }, ticks: { font: { family: 'Inter', size: 9 }, color: '#64748b', maxTicksLimit: 12 } }, y: { title: { display: true, text: 'Received From (OFFICE)', font: { family: 'Inter', size: 12, weight: '600', style: 'italic' }, color: '#475569' }, grid: { color: '#f1f5f9', drawBorder: false }, beginAtZero: true, ticks: { font: { family: 'Inter', size: 10 }, color: '#64748b' } } } 
        }
    });
}
...
function drawDonutChart(canvasId, labels, dataArr, grandTotal) {
    ...
    if(!canvas) return;
    const ctx = canvas.getContext('2d');
    if (monthlyTotalPieInstance) monthlyTotalPieInstance.destroy();
    ...
    monthlyTotalPieInstance = new Chart(ctx, {
        type: 'doughnut', 
        data: { labels: labels, datasets: [{ data: dataArr, backgroundColor: mappedVibrant, borderWidth: 0, borderRadius: 8, spacing: 5, hoverOffset: 15 }] },
        options: { 
            responsive: true, maintainAspectRatio: false, cutout: '55%', layout: { padding: 15 }, animation: { animateScale: true, animateRotate: true, duration: 800, easing: 'easeOutExpo' }, hover: { mode: 'index', animationDuration: 300 }, 
            plugins: { 
                legend: { display: false }, 
                datalabels: { color: '#ffffff', font: { weight: '800', family: 'Inter', size: 9 }, anchor: 'center', align: 'center', formatter: (value, context) => { let sum = context.chart.data.datasets[0].data.reduce((a, b) => a + b, 0); if (sum === 0) return ''; let pctStr = ((value * 100) / sum).toFixed(1); let pctFloat = parseFloat(pctStr); return pctFloat >= 8 ? pctStr + '%' : ''; } },
                tooltip: { ...sharedTooltipConfig, callbacks: { label: function(context) { let val = context.raw; let pct = grandTotal > 0 ? ((val / grandTotal) * 100).toFixed(1) : 0; return [`${val} Services Catered`, `vs Grand Total: ${pct}%`]; } } }
            } 
        }
    });
}
...
function processOperationsData(data) {
    try {
        ...
        let monthSet = new Set(); const monthlyAgg = {};
        
        data.forEach(row => {
            if(row['MONTH']) { 
                let m = String(row['MONTH']).trim().toUpperCase();
                if(!monthlyAgg[m]) monthlyAgg[m] = { vehicular:0, roadside:0, patient:0, medical:0, standby:0, others:0, clearing:0, firetruck:0, hauling:0, ledvan:0, grandTotal:0, total1st:0, total2nd:0, total3rd:0, totalOutside:0 };
        ...
        const labels = [];
        const vehicular = [], roadside = [], patient = [], medical = [], standby = [], others = [], clearing = [], firetruck = [], hauling = [], ledvan = [];
        const monthlyTotalServices = [];
        let total1st = 0, total2nd = 0, total3rd = 0, totalOutside = 0, overallGrandTotal = 0;

        monthOrder.forEach(m => {
            if(monthlyAgg[m]) {
                labels.push(m); monthSet.add(m);
                operationsMonthlyCache[m] = [ monthlyAgg[m].vehicular, monthlyAgg[m].roadside, monthlyAgg[m].patient, monthlyAgg[m].medical, monthlyAgg[m].standby, monthlyAgg[m].others, monthlyAgg[m].clearing, monthlyAgg[m].firetruck, monthlyAgg[m].hauling, monthlyAgg[m].ledvan ];
                vehicular.push(monthlyAgg[m].vehicular); roadside.push(monthlyAgg[m].roadside); patient.push(monthlyAgg[m].patient); medical.push(monthlyAgg[m].medical); standby.push(monthlyAgg[m].standby); others.push(monthlyAgg[m].others); clearing.push(monthlyAgg[m].clearing); firetruck.push(monthlyAgg[m].firetruck); hauling.push(monthlyAgg[m].hauling); ledvan.push(monthlyAgg[m].ledvan);
                monthlyTotalServices.push(monthlyAgg[m].grandTotal); overallGrandTotal += monthlyAgg[m].grandTotal;
                total1st += monthlyAgg[m].total1st; total2nd += monthlyAgg[m].total2nd; total3rd += monthlyAgg[m].total3rd; totalOutside += monthlyAgg[m].totalOutside;
                for(let i=0; i<10; i++) operationsMonthlyCache['all'][i] += operationsMonthlyCache[m][i];
            }
        });

        let referenceTotal = overallGrandTotal > 0 ? overallGrandTotal : (total1st + total2nd + total3rd + totalOutside);

        const updateKPI = (id, val, pctId) => {
            const el = document.getElementById(id);
            if(el) { el.innerText = val; document.getElementById(pctId).innerText = referenceTotal > 0 ? ((val / referenceTotal) * 100).toFixed(1) + '% of Grand Total' : '0%'; }
        };
        updateKPI('kpi-1st', total1st, 'pct-1st'); updateKPI('kpi-2nd', total2nd, 'pct-2nd'); updateKPI('kpi-3rd', total3rd, 'pct-3rd'); updateKPI('kpi-outside', totalOutside, 'pct-outside');

        renderTrendFooter('trend-vehicular', vehicular, labels, true); renderTrendFooter('trend-roadside', roadside, labels, false); renderTrendFooter('trend-patient', patient, labels, true); renderTrendFooter('trend-medical', medical, labels, true); renderTrendFooter('trend-standby', standby, labels, false); 
        renderTrendFooter('trend-others', others, labels, false); renderTrendFooter('trend-clearing', clearing, labels, false); renderTrendFooter('trend-firetruck', firetruck, labels, false); renderTrendFooter('trend-hauling', hauling, labels, false); renderTrendFooter('trend-ledvan', ledvan, labels, false);
        ...
        ['vehicularChart', 'roadsideChart', 'patientChart', 'medicalChart', 'standbyChart', 'othersChart', 'clearingChart', 'firetruckChart', 'haulingChart', 'ledvanChart'].forEach(id => { renderToggleableChart(id, 'bar', true); });

        const drop = document.getElementById('masterServiceMonthFilter');
        if(drop) { drop.innerHTML = '<option value="all">All Time</option>'; Array.from(monthSet).forEach(m => { let opt = document.createElement('option'); opt.value = m; opt.innerText = m; drop.appendChild(opt); }); }
        renderMasterServicePie('all');

    } catch (e) { console.error("FATAL ERROR in processOperationsData:", e); }
}
...
function renderToggleableChart(canvasId, type, isInitialLoad = false) {
    try {
        ...
        if (!isInitialLoad) container.classList.add('chart-fade-out');

        setTimeout(() => {
            const ctx = canvas.getContext('2d');
            if (toggleChartInstances[canvasId]) toggleChartInstances[canvasId].destroy();
            ...
            let chartColor = Array.isArray(dataObj.color) ? dataObj.color[0] : dataObj.color;

            toggleChartInstances[canvasId] = new Chart(ctx, {
                type: 'bar',
                data: { labels: recentLabels, datasets: [{ label: dataObj.labelText, data: recentData, backgroundColor: chartColor, maxBarThickness: 15, borderRadius: 3, borderWidth: 0 }] },
                options: {
                    indexAxis: 'y', responsive: true, maintainAspectRatio: false, animation: { duration: 700, easing: 'easeOutQuart' }, layout: { padding: { top: 5, right: 35, bottom: 5, left: 10 } }, 
                    plugins: { datalabels: { display: true, color: '#1e293b', align: 'right', anchor: 'end', font: { weight: '800', family: 'Inter', size: 10 } }, legend: { display: false }, tooltip: sharedTooltipConfig },
                    scales: { x: { display: false, beginAtZero: true, grace: '20%' }, y: { grid: { display: false, drawBorder: false }, ticks: { font: { family: 'Inter', size: 10, weight: '700' }, color: '#64748b' }, border: {display: false} } }
                }
            });

            if (!isInitialLoad) setTimeout(() => { container.classList.remove('chart-fade-out'); }, 50); 
        }, isInitialLoad ? 0 : 300); 
...
function renderMasterServicePie(monthFilter) {
    try {
        ...
        let filteredLabels = []; let filteredData = []; let mappedColors = [];

        for(let i=0; i<10; i++) {
            if(dataArr[i] > 0) { filteredLabels.push(serviceCategoryLabels[i]); filteredData.push(dataArr[i]); mappedColors.push(pieColorPalette[i % pieColorPalette.length]); }
        }

        if(filteredLabels.length === 0) { filteredLabels = ["No Data"]; filteredData = [1]; mappedColors = ["#e2e8f0"]; } 
        else {
            let combined = filteredLabels.map((l, i) => ({l, d: filteredData[i], c: mappedColors[i]}));
            combined.sort((a,b) => b.d - a.d);
            filteredLabels = combined.map(x => x.l); filteredData = combined.map(x => x.d); mappedColors = combined.map(x => x.c);
        }
        ...
        if(masterServicePieInstance) {
            masterServicePieInstance.data.labels = filteredLabels; masterServicePieInstance.data.datasets[0].data = filteredData; masterServicePieInstance.data.datasets[0].backgroundColor = mappedColors; masterServicePieInstance.update();
        } else {
            masterServicePieInstance = new Chart(ctx, {
                type: 'doughnut', 
                data: { labels: filteredLabels, datasets: [{ data: filteredData, backgroundColor: mappedColors, borderWidth: 0, borderRadius: 8, spacing: 5, hoverOffset: 15 }] },
                options: { responsive: true, maintainAspectRatio: false, cutout: '55%', layout: { padding: 15 }, animation: { animateScale: true, animateRotate: true, duration: 800, easing: 'easeOutExpo' }, hover: { mode: 'index', animationDuration: 300 }, 
                    plugins: { legend: { display: false }, tooltip: sharedTooltipConfig, datalabels: { color: '#ffffff', font: { weight: '800', family: 'Inter', size: 10 }, formatter: (value, context) => { if(context.chart.data.labels[0] === "No Data") return ""; let sum = context.chart.data.datasets[0].data.reduce((a,b)=>a+b,0); let p = (value/sum*100); return p >= 5 ? p.toFixed(1)+'%' : ''; } } } }
            });
        }
        ...
        if(leg) {
            ...
            if(filteredLabels[0] !== "No Data") {
                filteredLabels.forEach((lbl, i) => {
                    let item = document.createElement('div'); item.className = 'legend-item interactive-legend-item'; item.style.padding = '8px 0'; item.style.animationDelay = `${i * 0.04}s`;
                    item.innerHTML = `<div class="legend-color" style="background-color: ${mappedColors[i]};"></div><div class="legend-text" title="${lbl}">${lbl}</div><div class="legend-val">${filteredData[i]}</div>`;
                    item.onclick = function() {
                        if (masterServicePieInstance) {
                            masterServicePieInstance.toggleDataVisibility(i); masterServicePieInstance.update();
                            if (masterServicePieInstance.getDataVisibility(i)) item.classList.remove('hidden-slice'); else item.classList.add('hidden-slice');
                        }
                    };
                    leg.appendChild(item);
                });
            } else { leg.innerHTML = `<div style="padding:20px; color:#94a3b8; font-size:0.8rem;">No Data Available</div>`; }
        }
    } catch (e) { console.error("Master Pie Chart Crash:", e); }
}
...
function processDocumentsData(data) {
    ...
    if (rawDocumentsData && rawDocumentsData.length > 0) {
        for (let i = 0; i < Math.min(5, rawDocumentsData.length); i++) {
            let row = rawDocumentsData[i]; let keys = Object.keys(row);
            ...
            if (targetKey && row[targetKey] !== undefined && String(row[targetKey]).trim() !== '') {
                ...
                if (!isNaN(parsedVal)) { explicitNoAction = parsedVal; break; }
            }
        }
    }
    ...
    data.forEach(row => {
        ...
        let isSummaryRow = (row['TOTAL ACTION TAKEN (OVERALL)'] !== undefined && String(row['TOTAL ACTION TAKEN (OVERALL)']).trim() !== '') || (row['TOTAL REQUEST CATERED'] !== undefined && String(row['TOTAL REQUEST CATERED']).trim() !== '');
        let isBlankRow = (!rawNature || String(rawNature).trim() === '') && (!rawCategory || String(rawCategory).trim() === '') && (!dateStr || String(dateStr).trim() === '');
        ...
        if (!isSummaryRow && !isBlankRow) {
            ...
            if (actionTxt.includes('no action')) { dynamicKPIs.noAction++; } 
                ...
                actionActuallyTaken = true; dynamicKPIs.action++;
                if (actionTxt.includes('not catered')) dynamicKPIs.notCatered++;
                else if (actionTxt.includes('catered') || actionTxt === 'catered') dynamicKPIs.catered++;
                else if (actionTxt.includes('cancelled')) dynamicKPIs.cancelled++;
                else if (actionTxt.includes('not attended')) dynamicKPIs.invNotAttended++;
                else if (actionTxt.includes('attended')) dynamicKPIs.invAttended++;
                else dynamicKPIs.others++;
            } 

            let mappedNature = rawNature.trim(); let upperNature = mappedNature.toUpperCase();
            ...
            if (dateStr && String(dateStr).trim() !== '') {
                ...
                if (parsedDate) {
                    ...
                    let m = parsedDate.getMonth() + 1; let y = parsedDate.getFullYear();
                    monthYearKey = `${y}-${m.toString().padStart(2, '0')}`; uniqueMonths.add(monthYearKey);
                }
            }

            globalDocRecords.push({ dateKey: monthYearKey, level1: mappedNature, level2: subCategory, level3: specificOffice, hasActionTaken: actionActuallyTaken, count: 1 });
        }
    });

    originalKPITotals = { req: dynamicKPIs.req, action: dynamicKPIs.action, catered: dynamicKPIs.catered, notCatered: dynamicKPIs.notCatered, cancelled: dynamicKPIs.cancelled, invAttended: dynamicKPIs.invAttended, invNotAttended: dynamicKPIs.invNotAttended, others: dynamicKPIs.others, noAction: dynamicKPIs.noAction };
    ...
    if (monthSelect) {
        ...
        sortedMonths.forEach(my => {
            if(my === 'all') return;
            let [y, m] = my.split('-'); let dateObj = new Date(y, m - 1);
            let label = dateObj.toLocaleString('en-US', { month: 'long', year: 'numeric' });
            let opt = document.createElement('option'); opt.value = my; opt.innerText = label; monthSelect.appendChild(opt);
        });
    }
    renderDocPieChart(); renderLineChartByTimeframe('daily');
}
...
function updateTrackingKPIDisplays() {
    ...
    if (currentPieState.level === 1) {
        ...
        document.getElementById('doc-kpi-request').innerText = originalKPITotals.req; document.getElementById('doc-kpi-action').innerText = originalKPITotals.action; document.getElementById('doc-kpi-catered').innerText = originalKPITotals.catered; document.getElementById('doc-kpi-inv-att').innerText = originalKPITotals.invAttended; document.getElementById('doc-kpi-not-catered').innerText = originalKPITotals.notCatered; document.getElementById('doc-kpi-others').innerText = originalKPITotals.others; document.getElementById('doc-kpi-inv-not').innerText = originalKPITotals.invNotAttended; document.getElementById('doc-kpi-cancelled').innerText = originalKPITotals.cancelled; document.getElementById('doc-kpi-no-action').innerText = originalKPITotals.noAction;
    } else {
        let dynTotalRequestsMatched = 0; let dynActionsActuallyTakenMatched = 0; let targetCategory = currentPieState.level1Target;
        ...
        globalDocRecords.forEach(record => {
            if (currentPieState.filterKey === 'all' || record.dateKey === currentPieState.filterKey) {
                if (record.level1 === targetCategory) {
                    ...
                    if (record.hasActionTaken) dynActionsActuallyTakenMatched++;
                }
            }
        });

        document.getElementById('doc-kpi-request').innerText = dynTotalRequestsMatched; document.getElementById('doc-kpi-action').innerText = dynActionsActuallyTakenMatched;
        ...
        if (targetCategory === 'Request') { cardCatered.style.display = ''; cardNotCatered.style.display = ''; cardCancelled.style.display = ''; } 
        else if (targetCategory === 'Invitation') { cardInvAtt.style.display = ''; cardInvNot.style.display = ''; } 
        else if (targetCategory === 'Offer/Proposal' || targetCategory === 'For Information') { cardAction.style.display = ''; } 
        else { cardAction.style.display = ''; }
    }
}

function renderDocPieChart() {
    let sourceMap = {}; let hasData = false;

    globalDocRecords.forEach(record => {
        if (currentPieState.filterKey === 'all' || record.dateKey === currentPieState.filterKey) {
            if (currentPieState.level === 1) { sourceMap[record.level1] = (sourceMap[record.level1] || 0) + record.count; hasData = true; } 
            else if (currentPieState.level === 2 && record.level1 === currentPieState.level1Target) { sourceMap[record.level2] = (sourceMap[record.level2] || 0) + record.count; hasData = true; } 
            else if (currentPieState.level === 3 && record.level1 === currentPieState.level1Target && record.level2 === currentPieState.level2Target) { sourceMap[record.level3] = (sourceMap[record.level3] || 0) + record.count; hasData = true; }
        }
    });

    let sortedSources = [];
    if (!hasData) { sortedSources = [{ label: 'No Data Found', value: 1 }]; } 
    else { sortedSources = Object.keys(sourceMap).map(key => ({ label: key, value: sourceMap[key] })); sortedSources.sort((a, b) => b.value - a.value); }

    let labels = sortedSources.map(item => item.label); let dataValues = sortedSources.map(item => item.value);

    const titleEl = document.getElementById('pieChartTitle'); const backBtn = document.getElementById('pieBackButton');

    if (currentPieState.level === 1) { titleEl.innerText = 'NATURE OF LETTER'; if(backBtn) backBtn.style.display = 'none'; } 
    else if (currentPieState.level === 2) { titleEl.innerHTML = `BREAKDOWN: ${currentPieState.level1Target.toUpperCase()} <span style="color: #64748b; font-weight: 600; font-size: 0.65rem; opacity: 0.7; letter-spacing: 0.5px;">(CATEGORY OF REQUESTING/ WRITING PARTY)</span>`; if(backBtn) backBtn.style.display = 'block'; } 
    else if (currentPieState.level === 3) { titleEl.innerHTML = `BREAKDOWN: ${currentPieState.level2Target.toUpperCase()} <span style="color: #64748b; font-weight: 600; font-size: 0.65rem; opacity: 0.7; letter-spacing: 0.5px;">(SPECIFIC OFFICE / ENTITY)</span>`; if(backBtn) backBtn.style.display = 'block'; }
    ...
    if (docPieChartInstance) {
        ...
        docPieChartInstance.data.labels = labels; docPieChartInstance.data.datasets[0].data = dataValues; docPieChartInstance.data.datasets[0].backgroundColor = mappedColors;
        docPieChartInstance.update(); updateCustomLegend(labels, dataValues, !hasData);
    } else {
...
function drawInteractiveDonutChart(canvasId, labels, dataArr, isEmptyState = false) {
    ...
    docPieChartInstance = new Chart(ctx, {
        type: 'doughnut',
        data: { labels: labels, datasets: [{ data: dataArr, backgroundColor: mappedColors, borderWidth: 0, borderRadius: 8, spacing: 5, hoverOffset: isEmptyState ? 0 : 15 }] },
        options: {
            responsive: true, maintainAspectRatio: false, cutout: '55%', layout: { padding: 15 }, animation: { animateScale: true, animateRotate: true, duration: 800, easing: 'easeOutExpo' }, hover: { mode: 'index', animationDuration: 300 }, 
            onClick: (event, elements, chart) => {
                if (chart.data.labels.length === 1 && chart.data.labels[0] === 'No Data Found') return;
                if (elements[0]) {
                    const index = elements[0].index; const label = chart.data.labels[index];
                    if (currentPieState.level === 1) { currentPieState.level = 2; currentPieState.level1Target = label; renderDocPieChart(); } 
                    else if (currentPieState.level === 2) { currentPieState.level = 3; currentPieState.level2Target = label; renderDocPieChart(); }
                }
            },
            plugins: {
                legend: { display: false },
                datalabels: {
                    ...
                    formatter: (value, context) => { 
                        ...
                        let sum = context.chart.data.datasets[0].data.reduce((a, b) => a + b, 0); if (sum === 0) return ''; 
                        let pctStr = ((value * 100) / sum).toFixed(1); let pctFloat = parseFloat(pctStr); return pctFloat >= 8 ? pctStr + '%' : ''; 
                    } 
                },
                tooltip: {
                    filter: function(tooltipItem) { return tooltipItem.label !== 'No Data Found'; }, ...sharedTooltipConfig, 
                    callbacks: {
                        label: function(context) {
                            let suffix = ''; if (currentPieState.level < 3) { suffix = ' (Click to zoom)'; }
                            let activeNature = (currentPieState.level === 1) ? context.label : currentPieState.level1Target;
                            let unitStr = "Requests"; 
                            if (activeNature === 'Invitation') unitStr = 'Invitations'; else if (activeNature === 'For Information') unitStr = 'Information'; else if (activeNature === 'Offer/Proposal') unitStr = 'Offers/Proposals';
...
function updateCustomLegend(labels, data, isEmptyState = false) {
    const legendContainer = document.getElementById('customLegend'); if(!legendContainer) return;
    legendContainer.innerHTML = '';
    labels.forEach((label, index) => {
        let color = isEmptyState ? '#e2e8f0' : pieColorPalette[index % pieColorPalette.length]; let val = isEmptyState ? '-' : data[index];
        legendContainer.innerHTML += `<div class="legend-item" style="animation-delay: ${index * 0.04}s;"><div class="legend-color" style="background-color: ${color}"></div><div class="legend-text" title="${label}">${label}</div><div class="legend-val">${val}</div></div>`;
    });
}
...
function processTrainingsData(data) {
    ...
    globalTrainLineData = []; calDataMap = {}; let latestEventDateObj = null; let explicitTitleCounts = {};
    ...
    workingData.forEach(row => {
        ...
        if (cat && String(cat).trim() !== "") {
            ...
            if (dates) {
                ...
                if (parsedDate) {
                    if (!latestEventDateObj || parsedDate > latestEventDateObj) latestEventDateObj = parsedDate;
                    globalTrainLineData.push({ dateObj: parsedDate, rawDates: dates, title: titleSafe, agency: agencySafe, pax: pax, facilitator: facSafe, category: c, count: 1, timestamp: parsedDate.getTime(), participants: participantsSafe });
                }
            }
        }
        ...
        if (colI_Title && String(colI_Title).trim() !== "") {
            ...
            if (tName.toUpperCase() !== 'TRAINING/LECTURES') { let tFreq = parseInt(colJ_Freq) || 0; explicitTitleCounts[tName] = tFreq; }
        }
    });
    ...
    initCalendarControls(); renderCalendar();
...
function renderTrainingOverview(monthFilter) {
    let totalPax = 0; let categoryCounts = {}; let statusCounts = {}; let paxByCategory = {}; globalRemarksDetails = {}; 

    rawTrainingsData.forEach(row => {
        let dates = getRobustValue(row, ['INCLUSIVE DATES', 'DATES', 'DATE'], ['Column A']);
        if (monthFilter !== 'all') { let parsedDate = parseTrainingDate(dates); if (!parsedDate || monthOrder[parsedDate.getMonth()] !== monthFilter.toUpperCase()) return; }

        let cat = getRobustValue(row, ['CATEGORY'], ['Column B']);
        let paxRaw = getRobustValue(row, ['NO. PAX', 'PAX', 'NO PAX'], ['Column F']); let pax = parseInt(paxRaw) || 0;
        let status = getRobustValue(row, ['REMARKS', 'REMARK'], ['Column G']);
        let facilitatedBy = getRobustValue(row, ['FACILITATOR'], ['Column H']);

        if (cat && String(cat).trim() !== "") {
            totalPax += pax; let c = String(cat).trim().toUpperCase();
            categoryCounts[c] = (categoryCounts[c] || 0) + 1; paxByCategory[c] = (paxByCategory[c] || 0) + pax;

            if (status && String(status).trim() !== "") {
                let s = String(status).trim().toUpperCase();
                statusCounts[s] = (statusCounts[s] || 0) + 1;
                if (!globalRemarksDetails[s]) globalRemarksDetails[s] = [];
                globalRemarksDetails[s].push({ title: getRobustValue(row, ['TRAINING/LECTURE'], ['Column C']) || 'Unspecified Event', agency: getRobustValue(row, ['AGENCY/OFFICE', 'AGENCY', 'OFFICE'], ['Column D']) || 'N/A', dates: dates || 'N/A' });
            }
        }
    });

    const paxBox = document.getElementById('train-kpi-count'); if (paxBox) paxBox.innerText = totalPax.toLocaleString();
    ...
    let statusLabels = Object.keys(statusCounts); let statusData = Object.values(statusCounts);
    let statusColors = statusLabels.map(label => { if (label === 'WITH AAR') return '#10b981'; if (label === 'NO AAR') return '#f43f5e'; return '#94a3b8'; });
    drawTrainBarChart('trainStatusChart', statusLabels, statusData, statusColors); 
}

function initCalendarControls() {
    const timeFilter = document.getElementById('trainCalendarFilter'); const catFilter = document.getElementById('trainCategoryFilter'); 
    const btnPrev = document.getElementById('calPrevBtn'); const btnNext = document.getElementById('calNextBtn');
    
    if(timeFilter) { let newTimeFilter = timeFilter.cloneNode(true); timeFilter.parentNode.replaceChild(newTimeFilter, timeFilter); newTimeFilter.value = currentCalView; newTimeFilter.addEventListener('change', function(e) { currentCalView = e.target.value; renderCalendar(); }); }
    if(catFilter) { let newCatFilter = catFilter.cloneNode(true); catFilter.parentNode.replaceChild(newCatFilter, catFilter); newCatFilter.value = currentCalCategory; newCatFilter.addEventListener('change', function(e) { currentCalCategory = e.target.value; renderCalendar(); }); }
    if(btnPrev) { let newPrev = btnPrev.cloneNode(true); btnPrev.parentNode.replaceChild(newPrev, btnPrev); newPrev.addEventListener('click', function() { if(currentCalView === 'monthly') currentCalDate.setMonth(currentCalDate.getMonth() - 1); else if(currentCalView === 'quarterly') currentCalDate.setMonth(currentCalDate.getMonth() - 3); else if(currentCalView === 'yearly') currentCalDate.setFullYear(currentCalDate.getFullYear() - 1); renderCalendar(); }); }
    if(btnNext) { let newNext = btnNext.cloneNode(true); btnNext.parentNode.replaceChild(newNext, btnNext); newNext.addEventListener('click', function() { if(currentCalView === 'monthly') currentCalDate.setMonth(currentCalDate.getMonth() + 1); else if(currentCalView === 'quarterly') currentCalDate.setMonth(currentCalDate.getMonth() + 3); else if(currentCalView === 'yearly') currentCalDate.setFullYear(currentCalDate.getFullYear() + 1); renderCalendar(); }); }
}

function renderCalendar() {
    const container = document.getElementById('trainingCalendarContainer'); const label = document.getElementById('calCurrentLabel');
    if(!container || !label) return;
    
    calDataMap = {};
    globalTrainLineData.forEach(item => {
        if (currentCalCategory !== 'all' && item.category !== currentCalCategory) return; 
        let yyyy = item.dateObj.getFullYear(); let mm = String(item.dateObj.getMonth() + 1).padStart(2, '0'); let dd = String(item.dateObj.getDate()).padStart(2, '0');
        let dateKey = `${yyyy}-${mm}-${dd}`;
        if(!calDataMap[dateKey]) calDataMap[dateKey] = [];
        calDataMap[dateKey].push(item);
    });

    container.classList.remove('cal-anim-active'); container.classList.add('cal-anim-enter');
    
    setTimeout(() => {
        let html = ''; let viewClass = `cal-view-${currentCalView}`;
        let targetYear = currentCalDate.getFullYear(); let targetMonth = currentCalDate.getMonth();
        ...
        if (currentCalView === 'monthly') {
            ...
            let q = Math.floor(targetMonth / 3); let qStartMonth = q * 3;
            label.innerText = `Q${q + 1} ${targetYear}`;
            let m1 = buildMonthHTML(targetYear, qStartMonth, true); let m2 = buildMonthHTML(targetYear, qStartMonth + 1, true); let m3 = buildMonthHTML(targetYear, qStartMonth + 2, true);
            html = `<div class="cal-grid-container ${viewClass}">${m1}${m2}${m3}</div>`;
        } else if (currentCalView === 'yearly') {
            label.innerText = `${targetYear}`; let monthsHtml = '';
            for(let i = 0; i < 12; i++) monthsHtml += buildMonthHTML(targetYear, i, true);
            html = `<div class="cal-grid-container ${viewClass}">${monthsHtml}</div>`;
        }
        
        container.innerHTML = html; void container.offsetWidth;
        container.classList.remove('cal-anim-enter'); container.classList.add('cal-anim-active');
    }, 250); 
}

function buildMonthHTML(year, month, isSmallScale) {
    let daysInMonth = new Date(year, month + 1, 0).getDate(); let firstDay = new Date(year, month, 1).getDay(); 
    let html = `<div class="cal-month"><div class="cal-month-title">${monthOrder[month]}</div><div class="cal-weekdays"><div>S</div><div>M</div><div>T</div><div>W</div><div>T</div><div>F</div><div>S</div></div><div class="cal-days">`;
    for(let i=0; i<firstDay; i++) html += `<div class="cal-day empty"></div>`;
    
    for(let day=1; day<=daysInMonth; day++) {
        let padMonth = String(month + 1).padStart(2, '0'); let padDay = String(day).padStart(2, '0'); let dateKey = `${year}-${padMonth}-${padDay}`; let events = calDataMap[dateKey];
        if (events && events.length > 0) {
            let linesHtml = ''; let maxLines = 3; 
            for(let j=0; j<Math.min(events.length, maxLines); j++) { let lineClass = events[j].category === 'IEC LECTURE/ DRILL' ? 'cal-line-activity' : ''; linesHtml += `<div class="cal-line ${lineClass}"></div>`; }
            ...
            let tooltipListHtml = events.map((e, idx) => `
                <div style="margin-bottom:${idx === events.length-1 ? '0' : '8px'}; text-align:left;">
                    <div style="display:flex; align-items:flex-start; gap:6px; margin-bottom:4px;"><div class="cal-line ${e.category === 'IEC LECTURE/ DRILL' ? 'cal-line-activity' : ''}" style="width: 12px; height: 4px; flex: none; margin-top: 4px;"></div><span style="font-size:0.7rem; font-weight:800; color:#ffffff; line-height: 1.2;">${e.title}</span></div>
                    <div style="padding-left: 18px; font-size: 0.6rem; color: #cbd5e1; line-height: 1.5;"><div style="margin-bottom: 2px;"><span style="font-weight:700; color:#94a3b8;">Date:</span> ${e.rawDates}</div><div style="margin-bottom: 2px;"><span style="font-weight:700; color:#94a3b8;">Requesting Agency/Office:</span> ${e.agency}</div><div style="margin-bottom: 2px;"><span style="font-weight:700; color:#94a3b8;">Participants:</span> ${e.participants}</div><div style="margin-bottom: 2px;"><span style="font-weight:700; color:#94a3b8;">Total Pax:</span> ${e.pax}</div><div style="margin-bottom: 2px;"><span style="font-weight:700; color:#94a3b8;">Facilitator:</span> ${e.facilitator}</div></div>
                </div>`).join('');

            let tooltipHtml = `<div class="custom-tooltip cal-tooltip" style="z-index: 999; pointer-events: none; bottom: 110%; min-width: 220px; white-space: normal; padding: 12px;"><div style="color:#94a3b8; font-size:0.55rem; text-transform:uppercase; margin-bottom:10px; letter-spacing:0.5px; border-bottom:1px solid #334155; padding-bottom:6px;">${monthOrder[month]} ${day}, ${year}</div>${tooltipListHtml}</div>`;
            html += `<div class="cal-day has-event has-tooltip">${day}<div class="cal-event-indicator">${linesHtml}</div>${tooltipHtml}</div>`;
        } else { html += `<div class="cal-day">${day}</div>`; }
    }
    html += `</div></div>`;
    return html;
}
...
function drawTrainBarChart(canvasId, labels, dataArr, customColors = null) {
    const canvas = document.getElementById(canvasId); if(!canvas) return; const ctx = canvas.getContext('2d');
    if (!labels || labels.length === 0) { labels = ["No Data"]; dataArr = [0]; }
    ...
    window[canvasId + 'Inst'] = new Chart(ctx, {
        ...
        options: {
            responsive: true, maintainAspectRatio: false, layout: { padding: { top: 25, left: 10, right: 10, bottom: 0 } }, 
            ...
            scales: { x: { grid: { display: false }, ticks: { font: { family: 'Inter', size: 9 }, color: '#64748b' }, border: {display: false} }, y: { grid: { color: '#f1f5f9', drawBorder: false }, ticks: { font: { family: 'Inter', size: 10 }, color: '#94a3b8' }, beginAtZero: true, grace: '20%', border: {display: false} } }
        }
    });
}
...
function populateAllList(containerId, dataObj) {
    ...
    if (sorted.length === 0) { container.innerHTML = `<div style="color: #94a3b8; font-size: 0.7rem; padding: 10px;">No Data</div>`; return; }

    sorted.forEach((item, index) => {
        container.innerHTML += `<div class="legend-item" style="animation-delay: ${index * 0.04}s;"><div class="legend-text" title="${item.name}" style="flex: 1;">${index + 1}. ${item.name}</div><div class="legend-val">${item.count}</div></div>`;
    });
}

function populateModalList(dataObj) {
    const container = document.getElementById('modal-title-list'); if (!container) return; container.innerHTML = '';
    let sorted = Object.keys(dataObj).map(k => ({name: k, count: dataObj[k]})).sort((a,b) => b.count - a.count);
    if (sorted.length === 0) { container.innerHTML = `<div style="color: #94a3b8; font-size: 0.9rem; padding: 20px; text-align:center;">No Data Available</div>`; return; }

    sorted.forEach((item, index) => {
        container.innerHTML += `<div class="legend-item" style="animation-delay: ${index * 0.02}s;"><div class="legend-text" style="font-size: 0.85rem; padding-right: 15px;"><span style="color:#64748b; font-weight:800; margin-right:8px;">${index + 1}.</span> ${item.name}</div><div class="legend-val">${item.count}</div></div>`;
    });
}

function populateRemarksModal(detailsObj) {
    const container = document.getElementById('modal-remarks-list'); if (!container) return; container.innerHTML = '';
    if (!detailsObj || Object.keys(detailsObj).length === 0) { container.innerHTML = `<div style="color: #94a3b8; font-size: 0.9rem; padding: 20px; text-align:center;">No Data Available</div>`; return; }

    for (let status in detailsObj) {
        let items = detailsObj[status]; if(!items || items.length === 0) continue;
        ...
        items.forEach((item, index) => {
            html += `<div class="legend-item" style="padding: 12px 0; border-bottom: 1px solid #f8fafc; align-items: flex-start; animation-delay: ${index * 0.02}s;"><div class="legend-text" style="font-size: 0.8rem; white-space: normal; line-height: 1.4;"><span style="font-weight: 800; color: #1e293b;">${item.title}</span><br><span style="font-size: 0.7rem; color: #64748b;">${item.agency} &nbsp;|&nbsp; ${item.dates}</span></div></div>`;
        });
...
// ==========================================
// VOLUNTEERS PROCESSING & PICTOGRAM
// ==========================================
function processVolunteersData(data) {
    let totalOrgs = 0; let totalIndividualsInOrgs = 0; let standaloneIndividuals = 0;
    let orgList = []; globalOrgGenderMap = {}; 
    ...
    data.forEach(row => {
        ...
        if (standaloneCount > 0) standaloneIndividuals += standaloneCount;
        if (orgName && orgCount > 0 && !orgName.toUpperCase().includes('TOTAL')) {
            totalOrgs++; totalIndividualsInOrgs += orgCount; orgList.push({ name: orgName, count: orgCount });
        }
    });
    ...
    // Second Pass: Extract Gender Breakdown Logic using Fuzzy Matching
    data.forEach(row => {
        let keys = Object.keys(row);
        ...
        if (indOrg && indGender) {
            ...
            // Exact or Partial Match to official summary list
    ...
    orgList.forEach((org, index) => {
        let tr = document.createElement('tr'); tr.style.animationDelay = `${index * 0.03}s`;
        
        let tdName = document.createElement('td');
        tdName.innerHTML = `<div style="display:flex; align-items:center; gap:12px;"><span style="color:#94a3b8; font-weight:800; font-size:0.6rem;">${index + 1}</span><span>${org.name}</span></div>`;
        
        let tdCount = document.createElement('td'); let percentage = (org.count / maxCount) * 100;
        tdCount.innerHTML = `<div style="display:flex; align-items:center; gap:12px; width:100%;"><span style="width: 30px; font-weight:800;">${org.count.toLocaleString()}</span><div style="flex:1; height:6px; background:#f1f5f9; border-radius:3px; overflow:hidden;"><div style="height:100%; width:${percentage}%; background:linear-gradient(90deg, #06b6d4, #2563eb); border-radius:3px; transition: width 1s ease-in-out;"></div></div></div>`; 
        
        tr.appendChild(tdName); tr.appendChild(tdCount); tbody.appendChild(tr);
    });
...
function renderPictogram(orgName) {
    ...
    let displayMale = 0; let displayFemale = 0; let pctMale = 0; let pctFemale = 0;

    if (data.TallyTotal > 0) {
        // We have individual rows, so calculate the ratio and apply it to the official count
        ...
        // No gender breakdown available, but we have a total count
        container.innerHTML = `
